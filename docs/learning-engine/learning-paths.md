...
3. **Curriculum generation** (`process_document_to_curriculum`, Gemini) — sections become courses/lessons with teaching content. The course title and each lesson title are also written by Gemini from the actual content (not the uploaded filename or raw section headings); if the LLM call fails or omits a title, it falls back per-field to a heuristic (cleaned-up filename for the course, heading/first-line text for a lesson) — see [Backend Services](/documentation?page=backend/services). `_generate_course_quizzes` pre-generates each lesson's short quiz (3 questions) and each course's final assessment (6 questions). Counts tunable in `logic_params.curriculum_generation`.
