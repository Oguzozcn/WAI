...
Documents: `GET /documents`, `POST /validate`, `DELETE /documents/{filename}`, `GET /documents/{filename}/download` (streams the raw file with `Content-Disposition: attachment`; 404 if unknown — surfaced as a download button per row on `/learning-materials`), `GET /documents/{filename}/versions`, `POST /documents/{filename}/versions/{version}/restore`.
...
| `POST /projects/{id}/pages` | Add a page. Body `{role, user_id, display_name, mode, title?, content?, source_doc_id?}`. `mode=blank` and `mode=import` return `{status: "created", page_id, project}` immediately. `mode=ai_draft` runs the source through the `draft_team_doc_page` prompt template → `call_gemini_json` as a **background job** (returns `{status: "processing", job_id}` right away instead of blocking on the LLM call) and **falls back to a plain import** when the LLM is unavailable; poll `GET /jobs/{job_id}` for the result. Sequential per-project ids (`page-0001`); pages carry `source` + `drafted_by` (`manual`/`import`/`ai`) provenance. |
...
| `POST /projects/{id}/generate-documentation` | **Documentation Master.** Synthesizes the project's full documentation set from every source in `linked_sources` via `generate_project_documentation` (`src/services/documentation_service.py`) — the same function the ADK orchestrator calls from chat. Runs as a **background job**: 400 synchronously if no sources are linked, otherwise returns `{status: "processing", job_id}` immediately ("this can take a minute") and the LLM call happens after the response — poll `GET /jobs/{job_id}`, whose terminal `status` is `completed` (with `pages_written`) or `error` (with `message`; there's no sound deterministic fallback for writing a whole document from scratch). On success, replaces any pages from a previous synthesis run (`drafted_by == "ai_synthesis"`) while leaving manually-written/imported pages untouched. |
| `GET /jobs/{job_id}?department=…` | Poll an `ai_draft` page or `generate-documentation` job. Shares the same job store as `/api/kb/upload/status/{job_id}` (`DepartmentScopedStore.read_kb_job`); 404 if unknown. |
...
The frontend never blocks on `ai_draft`/`generate-documentation`: `frontend/js/sidebar.js`'s `WisdomSidebar.trackJob({job_id, kind, department})` registers the job in `localStorage` and a shared poller (running on every page, not just the one that started the job) surfaces completion as a toast + persistent notification-bell entry, and dispatches a `wai:job-done` window event so a page still open on the same project (e.g. `/team-documentation`) can refresh in place. See `docs/frontend/shared-modules.md`.
