...
class DataComplianceGate:
    ...
    Prevents automatic system-generated completions from granting authorizations without manual human validation.
    """

    @staticmethod
    def audit_state_transition(user_id: str, proposed_state: str, context: dict = None) -> dict:
        """
        Intercepts state transitions. If a system tries to automatically move a user to a completed/authorized
        state without a human admin signature, it blocks it and sets status to PENDING_VERIFIED_HUMAN_APPROVAL.
        """
        if proposed_state in ["passed", "completed", "authorized"]:
            # Check for human controller signature
